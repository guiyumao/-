  ...
  <div>
    <div class="page-header">
      <div>
        ...
        <p class="page-subtitle">把危化品入库、领用、归还和废液处理放在一条业务链里，避免审批、见证和库存回写彼此分离。</p>
      </div>
      <el-space>
    ...
    <div class="sample-showcase">
      <div class="sample-section-head">
        <div>
          ...
          <p class="section-desc">教师和学生申请都来自当前危化记录，和见证人、审批人放在一起看，不拆成孤立模块。</p>
        </div>
    ...
    <el-card class="card-panel" shadow="never">
      <el-table v-loading="loading" :data="records">
        <el-table-column label="危化品" min-width="160">
          <template #default="{ row }">{{ materialMap[String(row.hazardousMaterialId)] || row.hazardousMaterialId }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" min-width="110" fixed="right">
          <template #default="{ row }">
            <el-button v-if="authStore.hasPermission('hazardous_usage:edit')" size="small" type="danger" plain @click="deleteRecord(row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
      ...
      <el-form ref="formRef" :model="form" :rules="rules" label-width="150px">
        ...
        <el-form-item label="危化品" prop="hazardousMaterialId">
          <el-select v-model="form.hazardousMaterialId" filterable placeholder="请选择危化品" style="width: 100%" @change="handleInventoryDependencyChange">
            <el-option v-for="item in materialOptions" :key="item.id" :label="item.subLabel ? `${item.label}（${item.subLabel}）` : item.label" :value="item.id" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="库存批次" prop="batchNo">
          <el-select v-model="form.batchNo" :disabled="form.actionType === 1" placeholder="请选择批次" style="width: 100%" @change="handleBatchChange">
            <el-option v-for="item in inventoryOptions" :key="item.id" :label="`${item.batchNo} / 可用 ${item.availableQuantity}`" :value="item.batchNo" />
          </el-select>
          <div v-if="form.actionType === 1" class="hint-line">入库场景请直接填写新的批次号。</div>
        </el-form-item>
...
function actionTypeText(actionType: number) {
  if (actionType === 1) return '入库'
  if (actionType === 2) return '领用'
  if (actionType === 3) return '归还'
  return '废液处理'
}
...
async function loadOptions() {
  const [labs, materials, users] = await Promise.all([fetchLaboratoryOptions(), fetchHazardousMaterialOptions(), fetchUserOptions()])
...
async function handleInventoryDependencyChange() {
  ...
  if (form.actionType === 1 || !form.laboratoryId || !form.hazardousMaterialId) return
...
async function deleteRecord(row: HazardousUsageRecord) {
  const confirmed = await ElMessageBox.confirm(
    `确认删除 ${materialName(row.hazardousMaterialId)} 的危化品记录吗？删除后会影响库存和追溯链路。`,
    '确认删除危化品记录',
    { confirmButtonText: '删除', cancelButtonText: '取消', type: 'warning' },
  ).catch(() => false)
  if (!confirmed) return
  await deleteHazardousUsage(row.id)
  ElMessage.success('危化品记录已删除')
  await loadAll()
}
