  ...
  <div>
      ...
      <div class="record-grid">
        <el-card v-for="record in records" :key="record.id" class="repair-card" shadow="never">
          <div class="repair-head">
            <div>
              <div class="record-title">
                ...
                <el-tag :type="repairStatusType(record.repairStatus)" effect="light">{{ repairStatusText(record.repairStatus) }}</el-tag>
              </div>
          ...
          <div class="footer-row">
            ...
            <el-space v-if="authStore.hasPermission('equipment_repair:edit')" wrap>
              <el-button size="small" type="primary" plain @click="openUpdate(record)">更新状态</el-button>
    ...
    <el-dialog v-model="createVisible" class="repair-dialog" title="新增维修记录" width="760px">
      ...
      <el-form ref="createFormRef" :model="createForm" :rules="createRules" label-width="140px">
        ...
        <el-form-item label="来源说明">
          <el-input v-model="createForm.remarks" type="textarea" :rows="2" maxlength="255" placeholder="例如：来源于借用归还检查、现场点检单或教师报修电话" show-word-limit />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-space>
    ...
    <el-dialog v-model="updateVisible" class="repair-dialog" title="更新维修状态" width="760px">
      ...
      <el-form ref="updateFormRef" :model="updateForm" :rules="updateRules" label-width="140px">
        ...
        <el-form-item label="来源说明">
          <el-input v-model="updateForm.remarks" type="textarea" :rows="2" maxlength="255" placeholder="例如：来源于维修工单、检测报告或现场复核记录" show-word-limit />
        </el-form-item>
      </el-form>
      <template #footer>
        <el-space>
...
function nextActorText(record: EquipmentRepairRecord) {
  if (record.repairStatus === 1) return '等待分派维修人员'
  if (record.repairStatus === 2) return `${userName(record.repairUserId)} 继续处理`
  if (record.repairStatus === 3 && linkedCalibration(record)) return '关注校准确认与恢复可用'
  if (record.repairStatus === 3) return '设备可恢复使用，必要时安排校准'
  if (record.repairStatus === 4) return '提交报废或替代方案评估'
  return '等待处理'
}

function repairFeedbackTarget(record: EquipmentRepairRecord) {
  const borrow = linkedBorrow(record)
  if (borrow) return `${userName(borrow.borrowerUserId)} 与实验室管理员`
  return '设备台账与实验室负责人'
}

function actionHint(record: EquipmentRepairRecord) {
  if (record.repairStatus === 1) return '当前尚未开始维修，建议尽快分派维修角色。'
  if (record.repairStatus === 2) return '维修处理中，需持续回填开始时间、过程说明和费用。'
  if (record.repairStatus === 3) return '维修完成后，可继续安排校准或恢复借用。'
  if (record.repairStatus === 4) return '维修失败，建议同步实验室负责人评估报废或替换。'
  return '等待进一步处理。'
}
...
async function deleteRecord(row: EquipmentRepairRecord) {
  const confirmed = await ElMessageBox.confirm(
    `确认删除 ${equipmentName(row.equipmentId)} 的维修记录吗？删除后会影响借用异常和校准联动。`,
    '确认删除维修记录',
...
.repair-dialog :deep(.el-input__wrapper),
.repair-dialog :deep(.el-textarea__inner),
.repair-dialog :deep(.el-select__wrapper) {
