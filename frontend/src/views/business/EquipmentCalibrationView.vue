    ...
    <div>
        <div class="page-header">
            ...
            <el-space>
                ...
                <el-button v-if="authStore.hasPermission('equipment_calibration:edit')" type="primary" @click="openCreate">
                    新增校准
                </el-button>
            </el-space>
        </div>
        ...
        <div class="sample-showcase">
            ...
            <div class="sample-grid">
                <div class="sample-card teacher" v-if="teacherBorrowSample">
                ...
                <div class="sample-card student" v-if="studentBorrowSample">
                    ...
                    <div class="card-head">
                        <div>
                            <div class="record-title">
                                ...
                                <el-tag :type="calibrationStatusType(record.calibrationStatus)" effect="light">
                                    {{ calibrationStatusText(record.calibrationStatus) }}
                                </el-tag>
                            </div>
                            <p class="record-subtitle">
                                {{ laboratoryName(record.laboratoryId) }} · 证书编号：{{ record.certificateNo }}
                            </p>
                        </div>
                        <div class="next-box">
            ...
            <el-form ref="createFormRef" :model="createForm" :rules="createRules" label-width="150px">
                ...
                <el-form-item label="来源说明">
                    <el-input
                        ...
                        :rows="2"
                        maxlength="255"
                        placeholder="例如：来源于年度校准计划、维修后复检或到期提醒安排"
                        show-word-limit
                    />
                </el-form-item>
            </el-form>
            ...
            <el-form ref="confirmFormRef" :model="confirmForm" :rules="confirmRules" label-width="150px">
                ...
                <el-form-item label="来源说明">
                    <el-input
                        ...
                        :rows="2"
                        maxlength="255"
                        placeholder="例如：来源于第三方检测报告、校准证书或现场确认单"
                        show-word-limit
                    />
                </el-form-item>
            </el-form>
...
function nextActorText(record: EquipmentCalibrationRecord) {
    if (record.calibrationStatus !== 2) {
        return '等待校准角色确认结果'
    }
    if (record.calibrationResult === 2) {
        return '设备暂停借用，必要时转维修'
    }
...
function actionHint(record: EquipmentCalibrationRecord) {
    if (record.calibrationStatus !== 2) {
        return '当前尚未确认，提醒中心应继续提示相关角色完成处理。'
    }
    if (record.calibrationResult === 2) {
        return '校准不合格，建议同步维修或限制借用。'
    }
...
function linkedCalibrationAction(borrow?: EquipmentBorrowRecord) {
    const calibration = linkedCalibrationByBorrow(borrow)
    if (!calibration) {
        return '需要补充校准记录后再判断借用可用性'
    }
    return calibration.calibrationStatus === 2
        ? '校准已确认，设备可继续支撑申请链路'
        : '校准待确认，提醒中心需继续跟进相关角色'
}
...
.flow-card {
    ...
    background: linear-gradient(150deg, rgba(13, 18, 24, 0.99), rgba(25, 31, 39, 0.99));
...
.metric-card {
    ...
    background:
        radial-gradient(circle at top right, rgba(121, 184, 220, 0.22), transparent 42%),
        linear-gradient(160deg, rgba(16, 21, 28, 0.98), rgba(26, 33, 42, 0.98));
}
...
.sample-showcase {
    ...
    border: 1px solid rgba(116, 145, 155, 0.16);
    background:
        radial-gradient(circle at top right, rgba(84, 169, 255, 0.12), transparent 32%),
...
.calibration-card {
    ...
    background: linear-gradient(180deg, rgba(14, 18, 24, 0.98), rgba(24, 31, 39, 0.98));
